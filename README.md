...
- **Configurable Safety:** unrestricted by default, with an adjustable `--safe` profile, mandatory `--block-*` restrictions, and `--dry-run` previews. Each INI file can distinguish archive output from ordinary file writes. Explicit `.xffrc` files cannot arm their own execution or deletion actions; trusted `--allow-exec` arming never bypasses operation blocks. See [`--help=safety`](docs/design-safety.md) for the operation table and limits.
