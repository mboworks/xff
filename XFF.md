...
- `--summary[=<GROUP>]` - aligned count + size table (or --format=jsonl rows) instead of each match; repeatable _(global, xff)_
  ...
  - `compare` - selected comparison counts and percentages by status; requires `--compare`
...
`--summary` (or `--summary=compare`) appends counts and percentages of selected comparison results by status and a total, including zero counts for empty selections. Each paired path counts once; directory-only entries are omitted as in the status listing. Percentages use the selected total; `--summary-precision` controls decimals. `--format=jsonl` renders these summary rows as `group`, `count`, and numeric `percent` objects. Other summary groupings describe each input tree separately.
