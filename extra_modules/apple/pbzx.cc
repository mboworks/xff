...
class PbzxStream final : public vfs::ReadStream {
 public:
  explicit PbzxStream(std::unique_ptr<vfs::ReadStream> input) : input_(std::move(input)) {}
...
class PbzxSource final : public vfs::ReadSource {
 public:
  explicit PbzxSource(vfs::SharedReadSource source) : source_(std::move(source)) {}
  ...
  absl::StatusOr<std::unique_ptr<vfs::ReadStream>> Open() const override {
    ...
    return std::make_unique<PbzxStream>(std::move(stream));
  }
