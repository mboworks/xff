...
`--help=full --help-format=markdown` reference are all generated. So **every feature add or change carries
...
The generated `--help` / `--man` / `--help=full --help-format=markdown` then stay complete by construction.
