...
Run all tooling suites with `bazel test //tools:python_tests`; they also run under `bazel test //...`.
...
`bazel run //tools:repository_tooling_test` is the repository-wide integration check. It uses
`BUILD_WORKSPACE_DIRECTORY` to inspect every BUILD file, the extra-module registry, and test ownership
in the live checkout. Main and release CI run it explicitly. It is a `py_binary` using the same
hermetic interpreter, not a sandboxed or cached `py_test`; its assertions execute on every run.
No list of exported BUILD files or source snapshots needs maintenance. For direct script execution,
supply `--workspace=PATH` explicitly. Fixture-based unit suites remain under `bazel test`.
...
content-store payload is at most 1,000,000,000 bytes by default. ASan uses 3,000,000,000 bytes and TSan uses
